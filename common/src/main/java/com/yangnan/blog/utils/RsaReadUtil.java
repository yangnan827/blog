...
public final class RsaReadUtil {
     ...
     * @return
     */
    public static PrivateKey getPrivateKeyFromFile(InputStream pfxPath, String priKeyPass) {
        InputStream priKeyStream = null;
        try {
            priKeyStream = pfxPath;
            byte[] reads = new byte[priKeyStream.available()];
            log.info("priKeyStream=====:{} priKeyStream.available()====:{}", priKeyStream, priKeyStream.available());
            log.info("reads length=====:{}", reads.length);
